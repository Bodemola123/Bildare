...
const SettingsPage: React.FC<SettingsProps> = ({ closeSettingsModal }) => {
  const [activeTab, setActiveTab] = useState<"account" | "security">("account");

// When you want to load security:
const openAccountTab = () => setActiveTab("account");
const openSecurityTab = () => setActiveTab("security");
  ...
  const { username, interests, profile, referralCode, referralCount, userId, clearAuth, fetchSession } = useAuth();
    ...
    trackEvent("Delete_account_clicked");
  };

  const handleUpdateProfileClick = () => {
    trackEvent("Update_profile_clicked");
  }
...
<div className="fixed inset-0 z-50 bg-[#1C1D1990] flex items-center justify-center p-4">
  <div className="bg-[#1F201C] w-full md:max-w-[858px] max-h-[90vh] md:max-h-[650px] mx-auto flex flex-col rounded-2xl gap-8 p-6 sm:p-8">
    ...
    <div className="flex flex-col md:flex-row gap-6 items-start h-full">
      ...
      {/* Sidebar */}
      <div className="flex-shrink-0 flex md:flex-col gap-4 w-full md:w-[152px]">


        {/* Account */}
        <button
          onClick={openAccountTab}
          className={`flex items-center gap-4 py-2.5 transition rounded-xl cursor-pointer
            ${activeTab === "account" ? "text-[#B9F500]" : "text-white opacity-70 hover:opacity-100"}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-4 py-2.5 transition rounded-xl cursor-pointer
            ${activeTab === "security" ? "text-[#B9F500]" : "text-white opacity-70 hover:opacity-100"}`}
        >
      ...
      {/* Divider */}
      <div className="bg-[#757575] w-[1px] self-stretch hidden md:block"></div>

      {/* Main Content */}
      <div className="flex flex-col gap-6 w-full h-[450px] md:h-[500px] overflow-y-auto pb-4 scrollbar-hide">
        ...
        {activeTab === "account" ? (
          ...
          <>
            ...
            {/* Insert Security.tsx content here */}
