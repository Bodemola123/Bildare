...
const Card: React.FC<CardProps> = ({ title, subtitle, date, icon }) => {
  ...
  return (
    <div
      className="flex flex-col justify-between bg-[#292a25] py-6 px-5 rounded-2xl h-[387px] md:h-[562px] w-full gap-6 text-white max-w-[659px]"
      onClick={() => trackEvent('card_click', { title })}
    >
      ...
      <div className="flex flex-col items-center text-center gap-2.5">
        ...
        <p
          className="line-clamp-2 text-[#B0B0B0] font-normal text-base"
          style={{ lineHeight: '145%', letterSpacing: '-0.04em' }}
        >
          Meet AstroClash, a fully-featured Play-to-Earn NFT Marketplace Kit,
          including exclusive design files all editable in Figma plus a pack of
          custom, beautiful 3D illustrations...
        </p>
      </div>
    </div>
  )
          ...
          {['Help', 'Email', 'Twitter', 'Discord', 'Terms', 'Privacy'].map(
            (link) => (
              <p
                ...
                onClick={() => trackEvent('footer_link_click', { link })}
              >
                {link}
              </p>
            )
      ...
      <div className="fixed bottom-16 right-10 z-50 flex flex-col items-end space-y-2">
        {isOpen && (
          <div className="p-4 rounded-2xl flex flex-col items-start justify-between bg-[#1C1D19] text-white w-64 shadow-xl">
            <button
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Welcome to Bildare' })}
            >
              Welcome to Bildare
            </button>
            <button
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Try a Template' })}
            >
              Try a Template
            </button>
            <Link
              href="/helpcenter"
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Help Center' })}
            >
              Help Center
            </Link>
            <Link
              href="/blog"
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Our Blog' })}
            >
              Our Blog
            </Link>
            ...
            <button
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Legal Summary' })}
            >
              Legal Summary
            </button>
            <Link
              href="/feedback"
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Submit Feedback' })}
            >
              Submit Feedback
            </Link>
            <hr className="w-full my-2 border-[#333]" />
            <button
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Community' })}
            >
              Community
            </button>
            <Link
              href="/feedback"
              className="py-1 hover:text-[#B9F500]"
              onClick={() => trackEvent('floating_menu_item_click', { item: 'Report an Issue' })}
            >
              Report an Issue
            </Link>
          </div>
        ...
        <button
          onClick={() => {
            setIsOpen((prev) => !prev)
            trackEvent('get_started_click', { opened: !isOpen })
          }}
