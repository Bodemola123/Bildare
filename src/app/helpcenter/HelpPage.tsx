      ...
      <div className="flex flex-row items-center justify-between gap-4">
        ...
        {!username ? (
          <Button
            ...
            onClick={() => trackEvent("auth_click", { action: "login_register", page: "help_center" })}
          >
          ...
          <DropdownMenu>
            ...
            >
              <DropdownMenuItem
                onClick={() => {
                  trackEvent("auth_click", { action: "logout", page: "help_center" });
                  clearAuth();
                }}
          ...
          {[
            ...
            <div key={idx} className='flex flex-col gap-2'>
              ...
              {section.links.map((link, lidx) => (
                <p
                  key={lidx}
                  className='text-[#B0B0B0] font-normal text-sm hover:text-[#B9F500] cursor-pointer transition-colors'
                  onClick={() =>
                    trackEvent("help_sidebar_click", {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 text-xs font-medium text-white">
        ...
        <div className="flex flex-wrap gap-4 justify-start sm:justify-end items-center">
          {['Help', 'Email', 'Twitter', 'Discord', 'Terms', 'Privacy'].map((item) => (
            <p
              key={item}
              onClick={() => trackEvent("footer_link_click", { link: item, page: "help_center" })}
